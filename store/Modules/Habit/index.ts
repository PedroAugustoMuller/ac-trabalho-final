...
const userHabits = localStorage.getItem('habits')

export const useHabits = defineStore('habit', () => {
    async function fetchHabits(): Promise<Array<Habit>> {
        return userHabits ? JSON.parse(userHabits) : defaultHabits;
    }

    async function createhabits(novoHabit: Habit): Promise<Boolean> {
        return true;
    }
